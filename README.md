...
# SYNOPSIS 📖
**What can I do with this?** This image will run the Netbird client as a container. Generating a [setup key](https://docs.netbird.io/how-to/register-machines-using-setup-keys) is all you need to do.
...
| ---: | ---: | :---: | :---: | :---: |
| 11notes/netbird-client | 31MB | 0:0 | ❌ | amd64, arm64, armv7 |
| netbirdio/netbird | 52MB | 0:0 | ❌ | amd64, arm64, armv7 |
...
x-lockdown: &lockdown
  ...
  # needed for full netbird client to manipulate routing and DNS
  cap_add:
    - NET_ADMIN
    - SYS_ADMIN
    - SYS_RESOURCE
...
services:
  client:
    image: "11notes/netbird-client:0.64.2"
    <<: *lockdown
    environment:
      TZ: "Europe/Zurich"
      NETBIRD_PEER_NAME: "docker-client"
      NETBIRD_URL: "${NETBIRD_URL}"
      NETBIRD_SETUP_KEY: "${NETBIRD_SETUP_KEY}"
    volumes:
      - "netbird-client.etc:/netbird/etc"
    tmpfs:
...
* [0.64.2](https://hub.docker.com/r/11notes/netbird-client/tags?name=0.64.2)

### There is no latest tag, what am I supposed to do about updates?
It is my opinion that the ```:latest``` tag is a bad habbit and should not be used at all. Many developers introduce **breaking changes** in new releases. This would messed up everything for people who use ```:latest```. If you don’t want to change the tag to the latest [semver](https://semver.org/), simply use the short versions of [semver](https://semver.org/). Instead of using ```:0.64.2``` you can use ```:0``` or ```:0.64```. Since on each new version these tags are updated to the latest version of the software, using them is identical to using ```:latest``` but at least fixed to a major or minor version. Which in theory should not introduce breaking changes.

If you still insist on having the bleeding edge release of this app, simply use the ```:rolling``` tag, but be warned! You will get the latest version of the app instantly, regardless of breaking changes or security issues or what so ever. You do this at your own risk!

# REGISTRIES ☁️
```
docker pull 11notes/netbird-client:0.64.2
docker pull ghcr.io/11notes/netbird-client:0.64.2
docker pull quay.io/11notes/netbird-client:0.64.2
```
...
*created 28.01.2026, 06:37:46 (CET)*